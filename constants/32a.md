...
Equivalently, it is the smallest (i.e., infimum of) $\alpha\in\mathbb{R}$ satisfying that there exists $\beta>0$ such that for every jointly-distributed finite discrete random variables $X,Y$, we can construct a finite discrete random variable $S$ (jointly-distributed with $X,Y$) with $I(X;S)=0$ and $H(Y|S)\le I(X;Y)+\log_{2}(I(X;Y)+\beta)+\alpha$ [LEG18], [Li25].
